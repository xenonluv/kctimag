...
const page = (title: string, message: string, ok = false) =>
  new NextResponse(
    `<!doctype html><html lang="ko"><head><meta charset="utf-8">` +
      ...
      `<title>${title} · 주간 한국문화 AI 매거진</title></head>` +
      ...
      `<p style="margin:0 0 18px;font-size:12px;letter-spacing:2px;text-transform:uppercase;color:#3b82f6">주간 한국문화 AI 매거진</p>` +
...
export async function GET(req: NextRequest) {
  ...
  return page(
    "구독이 취소되었습니다",
    "이제부터 주간 한국문화 AI 매거진 소식을 보내드리지 않습니다.<br/>그동안 함께해 주셔서 진심으로 감사합니다.",
    true,
  );
}
